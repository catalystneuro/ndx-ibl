...
def main():
    ...
    trajectory_table = NWBGroupSpec(
        ...
        doc='Probe insertion trajectory parameters for a single probe. Each row represents a trajectory '
            ...
            '(histology refined using electrophysiology). The coordinate system is bregma-centered with '
            'units in micrometers: x is ML (medio-lateral, positive=right), y is AP (anterior-posterior, '
            'positive=anterior), z is DV (dorso-ventral, positive=dorsal). Depth is the distance along '
            'the probe axis from brain surface to tip. Angles define insertion direction: theta is polar '
            'angle from vertical (0=down, 90=horizontal), phi is azimuth from AP axis (0=anterior, 90=left). '
            'Roll defines electrode-facing direction.',
        datasets=[
            NWBDatasetSpec(
                name='device',
            ...
            NWBDatasetSpec(
                ...
                neurodata_type_inc='VectorData',
                dtype='float',
                doc="Entry point ML (medio-lateral) coordinate in micrometers, bregma-centered. "
                    "Positive values are right of midline, negative are left. For histology-derived "
                    "trajectories, computed from the intersection of the fitted probe track with brain surface."
            ),
            NWBDatasetSpec(
                name='y',
                neurodata_type_inc='VectorData',
                dtype='float',
                doc="Entry point AP (anterior-posterior) coordinate in micrometers, bregma-centered. "
                    "Positive values are anterior to bregma, negative are posterior. For histology-derived "
                    "trajectories, computed from the intersection of the fitted probe track with brain surface."
            ),
            NWBDatasetSpec(
                name='z',
                neurodata_type_inc='VectorData',
                dtype='float',
                doc="Entry point DV (dorso-ventral) coordinate in micrometers, bregma-centered. "
                    "Typically negative values (below bregma/brain surface). For histology-derived "
                    "trajectories, computed from the intersection of the fitted probe track with brain surface."
            ),
            NWBDatasetSpec(
                name='depth_um',
                ...
                doc="Insertion depth in micrometers, measured along the probe axis from the brain surface "
                    "entry point to the probe tip. For histology-derived trajectories, computed as the "
                    "Euclidean distance between entry point and the deepest traced point projected onto "
                    "the fitted trajectory line."
            ),
            NWBDatasetSpec(
                name='theta',
                neurodata_type_inc='VectorData',
                dtype='float',
                doc="Polar angle in degrees measuring probe tilt from vertical. 0 degrees means straight "
                    "down into brain (along DV axis), 90 degrees means horizontal. For histology-derived "
                    "trajectories, computed using spherical coordinates: theta = arccos(dz/depth) where dz "
                    "is the vertical component of the entry-to-tip vector."
            ),
            NWBDatasetSpec(
                name='phi',
                neurodata_type_inc='VectorData',
                ...
                doc="Azimuth angle in degrees defining the direction of probe tilt in the horizontal plane. "
                    ...
                    "(0 = tilted anteriorly, 90 = tilted left, 180 = tilted posteriorly). For histology-derived "
                    "trajectories, computed using spherical coordinates: phi = arctan2(dy, dx)."
            ),
            NWBDatasetSpec(
                name='roll',
                neurodata_type_inc='VectorData',
